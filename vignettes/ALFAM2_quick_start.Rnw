...
Model parameter values for the ``average'' ALFAM2 model can be fonud in the default value of the \texttt{pars} argument. 
...
So, for exapmle, the (secondary) parameter \texttt{wind.2m1}, which is 0.0487 s/m by default, is used in the calculation of the primary parameter $r_1$.
