...
This book for a course on R has some details: \url{https://www.researchgate.net/publication/325170649_An_Introduction_to_R_for_Beginners}.
...
The latest and earlier versions of the ALFAM2 package are available from a GitHub repository: \url{https://github.com/sashahafner/ALFAM2}.
...
Installation only needs to be carried out once.
...
\hline
  ...
  \texttt{app.rate.ni}   & Manure app. rate (no inj.) & t/ha           &                   &  \\
...
To help ensure that incorporation is indeed applied as intended\footnote{The information entered in the data frame is inconsistent for rows 1 and 4, with \texttt{incorp} set to \texttt{'none'} but \texttt{t.incorp} set to \texttt{4}. The function carries out incorporation only if \textit{both} variables indicate it should.} \texttt{warn = TRUE}.
...
Note that instantaneous flux (\texttt{jinst}) will always be lower than interval average flux when flux monotonically decreases over time.
...
But output is always a data frame, which can of course be changed to a data.table or a tibble after the \texttt{alfam2} call.
