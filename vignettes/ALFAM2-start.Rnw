...
We will need to add a new column with a grouping variable also, to let \texttt{ALFAM2mod()} know that each row represents a different scenario.
...
<<>>=
pred5b <- ALFAM2mod(dat5b, app.name = "TAN.app", time.name = "ctime", 
...
This data preparation option will automatically calculate dummy variables for application method, incorporation, and manure source (currently pig is the only level different from the reference).
...
To show that incorporation is indeed applied as intended\footnote{The information in \texttt{dat4b} is a bit inconsistent, with \texttt{incorp} set to \texttt{'none'} but \texttt{t.incorp} set to , let's set to \texttt{4}. The function carries out incorporation only if \textit{both} variables indicate it should.} \texttt{warn = TRUE}.
...
<<>>=
pred8 <- ALFAM2mod(dat8, app.name = 'TAN.app', time.name = 'ctime',
                   time.incorp = 't.incorp', warn = FALSE, add.incorp.rows = TRUE)
@

<<>>=
plot(e ~ ct, data = pred8, type = 'o', xlab = 'Time (h)', 
     ylab = 'Cumulative emission (kg/ha)')
abline(v = 6.5, col = 'blue')
plot(j ~ ct, data = pred8, type = 'S', log = 'y', col = 'red', 
     xlab = 'Time (h)', ylab = 'Average flux (kg/ha-h)')
abline(v = 6.5, col = 'blue')
@
