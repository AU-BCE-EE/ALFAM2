...
\begin{table}[]
  \caption{Default predictor variables that can be used with \texttt{ALFAM2mod()}, as given in the \texttt{ALFAM2pars02} or \textttt{ALFAM2pars01} objects.}
\begin{tabular}{lllll}
\hline
...
For information on how these values were calculated, see the report on calculation of Danish emission factors (Hafner et al. 2021).
(An earlier version (``Set 1'') are available in \texttt{ALFAM2pars01}. Derivation of these is described in Hafner et al. (2019).) 
...
We could also specify this behavior with \texttt{t.incorp = NA}.\footnote{Rhis would provide identical results, but for package v0.3.2 and prior, this approach did not work correctly unless \texttt{incorp.deep} was set to FALSE.}
