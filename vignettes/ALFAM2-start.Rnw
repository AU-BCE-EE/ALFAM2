...
But note that the \texttt{f4} mass transfer occurs at the beginning of an interval, and therefore shows up for the 72 h interval in the output, which takes place from 4 to 72 h after slurry application.
...
\subsection{Example 7. Faster evaluation}
...
<<>>=
system.time(
  pred9 <- alfam2(dat9, app.name = 'TAN.app', time.name = 'ct', group = 'field', warn = FALSE)
)
@
...
system.time(
  pred9b <- alfam2(dat9b, app.name = 'TAN.app', time.name = 'ct', 
                   time.incorp = "t.incorp", group = 'field', warn = FALSE)
)
@
...
A lot of effort has gone into the warnings and error messages returned by \texttt{alfam2()}, with the objective of making it easier to understand and fix problems with a call or the predictor variable data frame.
...
pred0 <- alfam2(dat6b, app.name = "TAN.app", time.name = "ctime", 
       time.incorp = "t.incorp", group = "acenarip", warn = FALSE)
@
...
<<error=TRUE>>=
pred0 <- alfam2(dat6, app.name = "TAN.app", time.name = "ctim", 
                time.incorp = TRUE, group = "scenario", warn = FALSE)
@
