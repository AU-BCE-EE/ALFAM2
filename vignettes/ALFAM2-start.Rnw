...
and install the ALFAM2 package from GitHub.\footnote{
  ...
  To get the latest version of the package, add the argument \texttt{ref = "dev"}.
}
...
\hline
  ...
  \texttt{app.rate.ni}   & Manure app. rate (no injection) & t/ha           &                   &  \\
...
We could also specify this behavior with \texttt{t.incorp = NA} (this would provide identical results, but for package v0.3.2 and prior, this approach did not work correctly unless \texttt{incorp.deep} was set to FALSE).
