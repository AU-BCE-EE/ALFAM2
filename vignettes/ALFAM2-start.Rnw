...
\subsection{Example 7. Confidence intervals}
...
They are given in the output with `.lwr` and `.upr` suffixes (for lower and upper).
...
So here all \texttt{lwr} and \texttt{upr} limits are \texttt{NA} for time = 0 h.
...
And then, for a 90\% confidence interval that includes uncertainty in both inputs (only dry here) and parameters, we can use \texttt{quatile()}:
...
<<eval=FALSE>>=
library(data.table)
dat1 <- data.table(ctime = 168, TAN.app = 50, man.dm = 8, 
                   air.temp = 20, wind.2m = 3, 
                   app.mthd.bc = TRUE)
dat1
@
...
<<eval=FALSE>>=
library(tibble)
dat1 <- tibble(ctime = 168, TAN.app = 50, man.dm = 8, 
                   air.temp = 20, wind.2m = 3, 
                   app.mthd.bc = TRUE)
dat1
class(dat1)
...
<<eval=FALSE>>=
pred1c <- alfam2(dat1, app.name = 'TAN.app', time.name = 'ctime')
class(pred1c)
class(pred1c)
@
