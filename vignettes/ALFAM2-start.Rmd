...
The model, which is described in detail in Hafner et al. (2019), is the focus of the ALFAM2 R package and this document.
...
It can be downloaded from here: <https://rstudio.com/products/rstudio/download/>.
...
The latest and earlier versions of the ALFAM2 package are available from a GitHub repository: <https://github.com/sashahafner/ALFAM2>.
...
By design, any time a predictor variable is omitted when `alfam2()` is called, the reference level or value is assumed for that variable.\footnote{
...
To help ensure that incorporation is indeed applied as intended\footnote{The information entered in the data frame is inconsistent for rows 1 and 4, with `incorp` set to `'none'` but `t.incorp` set to `4`. The function carries out incorporation only if *both* variables indicate it should.} `warn = TRUE}.
...
By default, multiple rows in `dat} are assumed to all belog to the same scenario (same plot, same emission trial).\footnote{This is the reason the `group} argument was needed above.}
