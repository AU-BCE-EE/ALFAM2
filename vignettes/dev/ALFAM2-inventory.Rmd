...
The `alfami()` function facilitates this type of work by
...
This document shows how to use the `alfami()` function through examples.
...
The inventory function is available (and at leat initially debugged) in v4.2.5 and up.
It is currently only included in the development branch publicly available through the ALFAM2 GitHub repo.
So users can install it with

```{r,eval=FALSE}
devtools::install_github('AU-BCE-EE/ALFAM2', ref = 'dev')
```

This assumes the devtools package has already been installed.
Eventually (after more testing and probably some revision) the function will be available in the main branch and later through CRAN.
...
In practice users might work at the district, county, state, or country level--the names of geographic scale is completely flexible for `alfami()`.
...
The `alfami()` function could handle this, but for now we will set these issues aside.

For inventory calculations, the `alfami()` function needs these inputs above along with a few settings.
...
```{r}
ei <- alfami(dat, time.name = 'time.h', app.tan.name = 'TAN.app.tot')
```
...
The `alfami()` function does not calculate simple average emission factors, although if those are of interest they could be calculated from disaggregated results.
...
To see more detailed ALFAM2 predictions from the `alfam2()` function, check the `predref` element, which has all the output from `alfam2()` applied to the input data given in the `alfami()` call.
...
By default, `alfami()` uses 100 "plausible" parameter sets (the `alfam2pars03var` object) that were developed using a bootstrap approach as described in the latest ALFAM2 model paper[^1].

The `alfami()` function can also incorporate uncertainty from predictor variables, as well as TAN application, using a Monte Carlo approach.
...
In the `alfami()` call we now need to indicate that uncertainty should be included for both parameters and variables.
...
```{r}
alfami(dat, time.name = 'time.h', app.tan.name = 'TAN.app.tot',
...
```{r}
aggregate(ei$emisuc[, c('man.dm', 'man.ph', 'TAN.app.tot')], 
          ei$emisuc[, c('zone', 'sector')], FUN = mean)
```
...
Notice how the standard deviation is constant for `man.ph`, even though mean values differ.
This is because the specified distribution settings given for uncertainty are interpreted as applying at the aggregated level.
We set this in the last column of `uset`:

```{r}
uset
```

Each uncertainty replication (iteration) therefore adjusts slurry pH by the same amount for all application events.
We can see that we approximately have our requested standard deviation (the exact value we see above) in fact at the aggregated level.

```{r}
sd(aggregate(ei$emisuc[, c('man.ph')], ei$emisuc[, c('uset'), drop = FALSE], FUN = mean)$x)
```

There is an alternative approach, but first, let's look at the other input variables.
We don't see a constant standard deviation for `man.dm` or `TAN.app.tot` because we indicated that it was *relative* standard deviation that should be constant in `uset`.
...
In fact we see constant relative standard deviation, despite very different mean values.
...
If we have uncertainty parameters for the disaggregated level, here our zone/sector combinations, we can specify that in the `uncert.settings` argument.


```{r}
usetd <- data.frame(pvar = c('man.dm', 'man.ph', 'air.temp', 'wind.sqrt', 'TAN.app.tot'),
...
We should expect similar (on average, identical) *disaggregated* results.

```{r}
eid <- alfami(dat, time.name = 'time.h', app.tan.name = 'TAN.app.tot',
              uncert = c('pars', 'vars'), uncert.settings = usetd)
```
...
Indeed, similar.
But aggregated results will differ, because we have now removed the (perfect) correlation in error among the different application events.
...
Confidence intervals for aggregated results are wider when the uncertainty distribution parameters apply at the aggregated level.
The difference will increase as the number of application events increases.
The aggregated distribution approach is probably the better one, and we will use it for the remainder of the examples.

Let's explore some other features related to uncertainty.
...
In `alfami()`, each row is assumed to be a separate application event unless the value of the column given in the `eventkey` argument says otherwise.
...
By default `alfami()` gives the totals.
...
```{r}
ei <- alfami(dat, time.name = 'time.h', app.tan.name = 'TAN.app.tot',
	      aggkey = 'year', uncert = c('pars', 'vars'), uncert.settings = uset)
ei$emisagg
ei$emistot
```
...
ei <- alfami(dat, time.name = 'time.h', app.tan.name = 'TAN.app.tot',
        ...
        uncert.settings = uset)

ei$emisagg
ei$emistot
```

If multiple aggregation combinations are needed, `alfami()` can handle this as well.
For example, to aggregate in three different ways:

```{r}
ei <- alfami(dat, 
             time.name = 'time.h', 
             app.tan.name = 'TAN.app.tot',
	           aggkey = list('year',
	                         'man.source',
	                         c('year', 'zone')),
             uncert = c('pars', 'vars'), 
...
Now the `emisagg` element is a list of multiple data frames.
