context("alfam2() accuracy and stability")

test_that("Time step has no effect on result", {
  
  dat0 <- data.frame(ctime = 48, TAN.app = 100)
  pred0 <- alfam2(dat0, app.name = "TAN.app", time.name = "ctime")
...
test_that("Time step has no effect on result even with incorporation", {
  ...
  dat0$t.incorp <- 4
  pred0 <- alfam2(dat0, app.name = 'TAN.app', time.name = 'ctime', time.incorp = 't.incorp')

  dat1 <- data.frame(ctime = 0:48, TAN.app = 100)
  dat1$incorpdeep <- TRUE
  dat1$t.incorp <- 4
  pred1 <- alfam2(dat1, app.name = 'TAN.app', time.name = 'ctime', time.incorp = 't.incorp')
...
test_that("add.incorp.rows adds a row", {
  ...
  dat0$t.incorp <- 4
  pred0 <- alfam2(dat0, app.name = 'TAN.app', time.name = 'ctime', time.incorp = 't.incorp', add.incorp.rows = TRUE)
